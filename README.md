...
| 6 | 图片 / 富文本 / 代码块 | TipTap 富文本编辑器：粗体、斜体、下划线、删除线、列表、引用、链接、图片（按钮/粘贴/拖拽上传）、代码块（15 种语言语法高亮，输入时实时预览，支持语言下拉切换） |
| 7 | 浅色 / 深色主题 | 侧栏一键切换浅色 → 深色 → 跟随系统，自动响应系统深浅色变化，偏好持久化且首屏无闪烁 |
| 8 | 群消息已读回执 | 用户在公共频道看到最新消息时自动上报已读游标，发送者可实时看到「N 人已读 / 未读」 |
| 9 | 滚动辅助 | 消息列表离开底部时浮现「↓ 最新消息」按钮，点击平滑滚回最新消息 |

图片粘贴体验：直接 Ctrl/⌘+V 即可把剪贴板图片（含截图工具）贴入输入框，支持一次多图，贴入后立即显示本地缩略预览并在上传完成后替换为服务端地址，上传未完成时禁止发送。
...
| `read:report(lastReadId) → ack` / `read:update` | 双向 | 群聊已读游标上报与广播（消息 DTO 带 `readCount` 已读人数） |
